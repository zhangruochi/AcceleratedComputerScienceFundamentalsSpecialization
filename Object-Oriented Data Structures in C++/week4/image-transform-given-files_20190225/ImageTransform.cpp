...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  ...
  for (unsigned x = 0; x < image.width(); x++)
  {
    for (unsigned y = 0; y < image.height(); y++)
    {
      ...
      dis = sqrt(pow(abs( (double)x - (double)centerX ), 2 ) + pow(abs( (double)y - (double)centerY ), 2 ) );
      if(dis <= 160){
        pixel.l *= (1.0 - dis * 0.005);
      }else{
        pixel.l *= 0.2;
      }
    }
...
**/
    PNG illinify(PNG image)
    {
      ...
      double mid_1 = (illini_blue - illini_orange) / 2 + illini_orange;
      double mid_2 = mid_1 + 180;
      ...
      for (unsigned x = 0; x < image.width(); x++)
      {
        for (unsigned y = 0; y < image.height(); y++)
        {
          ...
          // which means you're changing the image directly. No need to `set`
          // the pixel since you're directly changing the memory of the image.
          if(pixel.h >= mid_1 && pixel.h < mid_2){
            pixel.h = illini_blue;
          }
          else
          {
    ...
    PNG watermark(PNG firstImage, PNG secondImage)
    {
      ...
      for(unsigned i = 0 ; i < min_width; i++){
        for(unsigned j = 0 ; j < min_height; j++){
          ...
          if (secondPixel.l != 0.0){
              HSLAPixel &firstPxiel = firstImage.getPixel(i,j);
              firstPxiel.l += 0.2;
          }
        }
      }
