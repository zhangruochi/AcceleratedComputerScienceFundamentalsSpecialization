...
static void treeFactory(GenericTree<int>& tree) {
  ...
  root -> data = 4;
  GenericTree<int>::TreeNode* root_left_child = root->addChild(8);
  GenericTree<int>::TreeNode* root_right_child = root->addChild(15);


  GenericTree<int>::TreeNode *root_left_left_child = root_left_child->addChild(16);
  root_left_child->addChild(23);

  root_left_left_child -> addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while(!que.empty()){
    ...
    que.pop();
    for (TreeNode* child : cur -> childrenPtrs){
      que.push(child);
    }
  }
